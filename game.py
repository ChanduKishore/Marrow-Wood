...
class Logs():
    ...
    def update_log(self, content):
        ...
        previous_log = previous_logs[:10] if previous_logs else ''
...
class GameWindow(RenderUtils,DialogBox,Inventory):
    def __init__(self, width, height, time_obj, followers, log):
        ...
        self.inventory = Inventory(self.half_screen_width,self.height-self.navigation_height,self.half_screen_width,self.navigation_height,self.current_selection.inventory,self.render_text,self.render_surface,self.screen)
    ...
    def inventory_screen(self,width, height, x, y, color ='#0B3041'):
        ...
        # self.render_text('This section will render Inventory screen',(x,y), color='#64FCF8')


    def surroundings_screen(self,width, height, x, y, color ='#0B3041'):
        self.render_surface(width, height, x, y, color)
...
time_object = DayNightCycle(239)



#set up character objects
person_stats = {'name':'First Follower', 'health':100, 'food':50, 'water':50, 'max_health':100, 'max_food':100, 'max_water':100, 'last_ate':0, 'last_drink': 0, 'inventory':[] }
person = CharacterStats(**person_stats)
apple = Fruit('Apple', 20, 50)
orange = Fruit('Orange', 10, 50)

# person.add_to_inventory(apple)
# person.add_to_inventory(orange)

follower_objects = [person]
log = Logs(time_object)

hunger_mechanism = HungerMechanism(person, time_object, log)

game = GameWindow(720,480, time_object, follower_objects, log)
